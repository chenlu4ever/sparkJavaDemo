...
public class MovieRecommand {
    public static void main(String[] args) {
        ...
        JavaRDD<String> ratingDate = javaSparkContext.textFile(fileUrl + "ratings.dat");

        // 所有评分数据，由于此数据要分三部分使用，60%用于训练，20%用于验证，最后20%用于测试。将时间戳%10可以得到近似的10等分，用于三部分数据切分
        JavaRDD<Tuple2<Integer, Rating>> ratingsTrain_KV = ratingDate.map(new Function<String, Tuple2<Integer, Rating>>() {
...
//        Tuple2<Integer, Rating> first = ratingsTrain_KV.first();
//        System.out.println("key:" + first._1+",value:"+ JSONObject.toJSONString(first._2));


        System.out.println("get " + ratingsTrain_KV.count() + " ratings from " + ratingsTrain_KV.distinct().count() + "users on " + ratingsTrain_KV.distinct().count() + "movies");

        // 加载我的评分数据
        JavaRDD<String> mydata = javaSparkContext.textFile(fileUrl + "myrating.dat");
        ...
        for (int i = 0; i < ranks.size(); i++) {
            for (int j = 0; j < lambdas.size(); j++) {
                for (int k = 0; k < iters.size(); k++) {
                    //训练获得模型
                    MatrixFactorizationModel model = ALS.train(JavaRDD.<Rating>toRDD(traningData_Rating), ranks.get(i), iters.get(i), lambdas.get(i));
        ...
        //加载电影数据
        JavaRDD<String> moviedata = javaSparkContext.textFile(fileUrl+"movies.dat");
